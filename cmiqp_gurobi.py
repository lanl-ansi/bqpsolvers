...
def main(args):
    ...
    for var in variables:
        gurobi_variables[var] = m.addVar(lb=0, ub=1, vtype=GRB.BINARY, name='group_%04d' % var)
    m.update()

    obj = 0.0
    for i, coeff in linear.items():
        obj += coeff * gurobi_variables[i]

    for (i, j), coeff in quadratic.items():
        obj += coeff * gurobi_variables[i] * gurobi_variables[j]
